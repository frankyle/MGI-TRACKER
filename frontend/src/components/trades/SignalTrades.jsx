...
const SignalTrades = () => {
  ...
  return (
    <>
      <Heading title='Entry Signals' subtitle='After anaylsis these are the entries that we are taking.' />
      <div className="content grid3 mtop">
    
      {candles
        .filter((candle) => candle.entry_candle)
        .sort((a, b) => b.is_active - a.is_active || b.id - a.id)
        .slice(0, 8)
        .map((candle, index) => {
          const { entry_candle, currency_pair, is_active } = candle;
         return (
